defmodule Bonfire.Files.Blurred do
  ...
  def blurred(_definition, original_path) when is_binary(original_path) do
    ...
    if String.starts_with?(path, "http") or is_nil(path) or path =="" or not File.exists?(path) or System.get_env("CI") do
      ...
      if File.exists?(blurred_path) do
        ...
        debug(blurred_path, "first time trying to get this blurred jpeg?")

        with saved_path when is_binary(saved_path) <- Bonfire.Files.Image.Edit.blur(path, blurred_path) do
