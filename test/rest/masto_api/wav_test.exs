defmodule Bonfire.Files.MastoApi.WavTest do
  ...
  test "WAV upload publishes an audio attachment even with an untrusted client MIME", %{conn: conn} do
    ...
    upload = %Plug.Upload{path: @wav, filename: "tone.wav", content_type: "application/octet-stream"}

    media = conn |> put_req_header("content-type", "multipart/form-data") |> post("/api/v1/media", %{"file" => upload}) |> json_response(200)
    assert media["type"] == "audio"
    assert {:ok, %{media_type: "audio/wav"}} = Bonfire.Files.Media.one(id: media["id"])

    status = conn |> post("/api/v1/statuses", %{"status" => "WAV playback", "visibility" => "public", "media_ids" => [media["id"]]}) |> json_response(200)
